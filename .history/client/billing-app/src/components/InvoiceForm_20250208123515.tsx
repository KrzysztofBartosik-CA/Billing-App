...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ invoice = { userId: '', date: '', lineItems: [] }, setInvoiceData, onSubmit, loading }) => {
    ...
    const handleLineItemChange = (index: number, field: keyof LineItem, value: string | number) => {
        ...
        if (setInvoiceData) {
            setInvoiceData({ ...invoice, lineItems: newLineItems, totalAmount: total, status: invoice.status || 'unpaid', invoiceNumber: invoice.invoiceNumber || '', _id: invoice._id || '' });
        }
    };
        ...
        return (
            <TextField
                ...
                onChange={(e) => {
                    ...
                    if (setInvoiceData) {
                        setInvoiceData({ ...invoice, userId: e.target.value, totalAmount: total, status: invoice.status || 'unpaid', invoiceNumber: invoice.invoiceNumber || '', _id: invoice._id || '' });
                    }
                }}
    ...
    const renderDatePicker = () => (
        <TextField
            ...
            onChange={(e) => {
                ...
                if (setInvoiceData) {
                    setInvoiceData({ ...invoice, date: e.target.value, totalAmount: total, status: invoice.status || 'unpaid', invoiceNumber: invoice.invoiceNumber || '', _id: invoice._id || '' });
                }
            }}
            error={dateError}
