...
const RefundRequests: React.FC = () => {
    ...
    const renderRefundRequestRow = (request: RefundRequest) => {
        const invoiceLink = request.invoice ? <Link to={`/invoice/${request.invoice?._id}`}>
        {request.invoice.invoiceNumber}
    </Link> : {i18n('invoice_deleted')};
        ...
        return (
            <TableRow key={request.id}>
                ...
                <TableCell>
                    ...
                    <Button variant="contained" color="secondary" onClick={() => handleDiscard(request.id)} style={{ marginLeft: '8px' }}>
                        {i18n('discard')}
                    </Button>
                </TableCell>
            </TableRow>
