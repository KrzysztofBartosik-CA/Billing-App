...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ invoice, setInvoiceData, onSubmit, loading }) => {
    ...
    const [lineItems, setLineItems] = useState<LineItem[]>(invoice?.lineItems && invoice.lineItems.length > 0 ? invoice.lineItems : [{
        description: '',
        quantity: 0,
        price: 0,
        total: 0,
    ...
    const [isEdition, setIsEdition] = useState(invoice ? true : false);
    ...
    const handleLineItemChange = (index: number, field: keyof LineItem, value: string | number) => {
        ...
        if (setInvoiceData) {
            setInvoiceData({ ...newInvoice, lineItems: newLineItems });
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        onSubmit({ ...newInvoice, userId: selectedUser, date, totalAmount: total, lineItems });
    };
        ...
        return (
            <TextField
                ...
                onChange={(e) => {
                    ...
                    if (setInvoiceData) {
                        setInvoiceData({ ...newInvoice, userId: e.target.value });
                    }
                }}
    ...
    const renderDatePicker = () => (
        ...
        const formattedDate = invoice?.date ? formatDate(invoice.date) : '';
        ...
        <TextField
            ...
            onChange={(e) => {
                ...
                if (setInvoiceData) {
                    setInvoiceData({ ...newInvoice, date: e.target.value });
                }
            }}
            error={dateError}
