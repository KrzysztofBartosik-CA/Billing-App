...
export const getRefundRequests = async (req: Request, res: Response) => {
    try {
        ...
        const refundRequestDTOs = await Promise.all(refundRequests.map(async (request) => {
            ...
            return new RefundRequestDTO(user as unknown as UserType, invoice as unknown as InvoiceType, request.reason, request.requestDate);
        }));
...
export const getRefundRequestById = async (req: Request, res: Response) => {
    try {
        ...
        const refundRequestDTO = new RefundRequestDTO(user as UserType, invoice as InvoiceType, refundRequest.reason, refundRequest.requestDate);
