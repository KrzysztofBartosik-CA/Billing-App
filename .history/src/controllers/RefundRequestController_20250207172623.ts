...
export const getRefundRequests = async (req: Request, res: Response) => {
    try {
        ...
        const refundRequestDTOs = await Promise.all(refundRequests.map(async (request) => {
            ...
            return new RefundRequestDTO(request.id, formattedUser as unknown as UserType, invoice as unknown as InvoiceType, request.reason, request.requestDate);
        }));
...
export const getRefundRequestById = async (req: Request, res: Response) => {
    try {
        ...
        const refundRequestDTO = new RefundRequestDTO(refundRequest.id, user as unknown as UserType, invoice as unknown as InvoiceType, refundRequest.reason, refundRequest.requestDate);
...
export const acceptRefundRequest = async (req: Request, res: Response) => {
    try {
        ...
        refundRequest.responseDate = new Date();
        await refundRequest.save();

        // Delete the refund request
        await RefundRequest.findByIdAndDelete(req.params.id);

        res.status(200).json({ message: 'Refund request accepted and deleted successfully' });
    } catch (error) {
