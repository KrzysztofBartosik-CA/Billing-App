...
import {ChangeRequestType, ChangeRequestTypeResponse} from '..types/changeRequestTypes.ts';
...
export const acceptChangeRequest = async (req: Request, res: Response): Promise<void> => {
    try {
        const changeRequest: ChangeRequestTypeResponse = await ChangeRequest.findById(req.params.id);
        if (!changeRequest) {
            res.status(404).json({ message: 'Change Request not found' });
            return;
        }
        ...
        invoice.userId = changeRequest.updatedInvoice.userId;
